...
<html lang="en" >
<head>
  <title>FineDeals.bootcamp/offers</title>
...
<body class="hero-anime">	
	...
	<div class="section full-height">
		<div class="absolute-center">
			<div class="container">
  <form id="upload_form" action="${pageContext.request.contextPath}/OfferController" method="post">
    	<div class="row">
    		<h4>Upload Offers</h4>
    		...
    		<div class="input-group input-group-icon">
    			<input type="text" id="comp_off" placeholder="Offer Percentage (%)" name="percentage" >
    			<span class="error_form" id="off_error_message"></span>
    		</div>
    		<div class="input-group input-group-icon">
    			<input type="text" id="comp_prname" placeholder="Product Name" name="prname">
    			<span class="error_form" id="prname_error_message"></span>
    		</div>
    		<div class="input-group input-group-icon">
    			<input type="text" id="comp_des" placeholder="Product Description" name="description">
      ...
      <button>Submit</button>
		...
		<script type="text/javascript">
    		$(function(){
    			...
    			$("#off_error_message").hide();
    			$("#prname_error_message").hide();
    			...
    			$("#comp_off").focusout(function(){
    				check_off();
    			});
    			$("#comp_prname").focusout(function(){
    				check_prname();
    			});
    			...
    			function check_off(){
    				...
    				var fname = $("#comp_off").val()
    				if(fname == ''){
    					$("#comp_off").css("border-bottom","2px solid #F90A0A");
    					error_off = true;
    				}
    				else if(pattern.test(fname) && fname !== '') {
    					$("#off_error_message").hide();
    					$("#comp_off").css("border-bottom","2px solid #34F458");
    				}else{
    					$("#off_error_message").html("should be between 0 and 100");
    					$("#off_error_message").show();
    					$("#comp_off").css("border-bottom","2px solid #F90A0A");
    					error_off = true;
    				}
    			}
    			
    			function check_prname(){
    				var pattern = /^[a-zA-Z0-9]*$/;
    				var fname = $("#comp_prname").val()
    				if(fname == ''){
    					$("#comp_prname").css("border-bottom","2px solid #F90A0A");
    					error_prname = true;
    				}
    				else if(pattern.test(fname) && fname !== '') {
    					$("#prname_error_message").hide();
    					$("#comp_prname").css("border-bottom","2px solid #34F458");
    				}else{
    					$("#prname_error_message").html("enter valid name");
    					$("#prname_error_message").show();
    					$("#comp_prname").css("border-bottom","2px solid #F90A0A");
    					error_prname = true;
    				}
    			}
    			...
    			$("#upload_form").submit(function(){
        			...
        			if(error_name == false && error_url == false && error_off == false && error_prname == false && error_des == false && error_ex == false){
